...
internal suspend fun Connection.connect(
    ...
    val connectionScope = CoroutineScope(job + Dispatchers.Unconfined + ignoreExceptionHandler)
    ...
    connectionScope.launch(start = CoroutineStart.UNDISPATCHED) {
        while (true) {
            if (!keepAliveHandler.tick(job)) continue
