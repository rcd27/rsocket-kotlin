...
internal class KeepAliveHandler(private val keepAlive: KeepAlive) {
    ...
    suspend fun tick(job: Job): Boolean {
        ...
        if (currentMillis() - lastMark.value < keepAlive.maxLifetimeMillis) return false
        //TODO
        job.cancel("Keep-alive failed", RSocketError.ConnectionError("No keep-alive for ${keepAlive.maxLifetimeMillis} ms"))
        return true
    }
}
